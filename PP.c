...
void add(){
   ...
   system("cls");         /*To clear screen*/
    ...
    f = fopen("record.txt","ab+");  /*Used to open the file*/
 ...
 if(f==NULL){
   ...
   fflush(stdin);   /*To clear first input*/
   while(another=='y'){
   ...
   fclose(f);      /*Used to close the file*/
   gotop(12,19);
   printf("press any key to continue.");
   getch();
   menu();
...
void modify(){
 ...
 if(f==NULL){
  ...
  //rewind(f);        /*Sets the file posn to the begining of file */  
fflush(stdin);
while(fread(&std,sizeof(std),1,f)==1){

    if(strcmp(sname,std.name)==0){         /*strcmp is used to compare  name of input sudent and name of Student in structure */
